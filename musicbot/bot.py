    ...
    def owner_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if not orig_msg or orig_msg.author.id == self.config.owner_id:
                ...
                raise exceptions.PermissionsError("only the owner can use this command", expire_in=60)
    ...
    def dev_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if orig_msg.author.id in self.config.dev_ids:
                ...
                raise exceptions.PermissionsError("only dev users can use this command", expire_in=60)
    ...
    async def _check_ignore_non_voice(self, msg):
        ...
        if not vc or vc == msg.author.voice_channel:
            ...
            raise exceptions.PermissionsError(
                "you cannot use this command when not in the voice channel (%s)" % vc.name, expire_in=60)
        ...
        async with self.aiolocks[_func_() + ':' + server.id]:
            if deserialize:
                ...
                if player:
                    ...
                    # Since deserializing only happens when Lida starts, I should never need to reconnect
                    return self._init_player(player, server=server)

            if server.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        'Lida is not in a voice channel.  '
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                ...
                newmsg = 'Now playing: **%s**' % (entry.title)
...
#######################################################################################################################
    ...
    async def on_ready(self):
        ...
        if owner and self.servers:
            ...
            if self.user.bot:
                log.warning(
                    "To make Lida join a server, paste this link in your browser. \n"
                    "Note: You should be logged into your main account and have \n"
                    "manage server permissions on the server you want Lida to join.\n"
    ...
    async def cmd_blacklist(self, message, user_mentions, option, something):
        ...
        if not user_mentions:
            raise exceptions.CommandError("No users listed.", expire_in=60)

        if option not in ['+', '-', 'add', 'remove']:
            raise exceptions.CommandError(
                'Invalid option "%s" specified, use +, -, add, or remove' % option, expire_in=60
            )
    ...
    async def cmd_lida(self, message):
        ...
        lida_picture = random.choice(list(open(self.config.lida_images_file)))

        embed = discord.Embed()
        embed.set_image(url=lida_picture)

        await self.send_message(message.channel, embed=embed)

    # !hani
    async def cmd_hani(self, message):
        """
        Usage:
            {command_prefix}lida

        Displays a random picture of Hani.
        """

        hani_picture = random.choice(list(open(self.config.hani_images_file)))

        embed = discord.Embed()
        embed.set_image(url=hani_picture)
    ...
    async def cmd_info(self, message):
        ...
        embed.set_thumbnail(url='https://i.shikashi.me/0Ln0V')
    ...
    async def cmd_joinserver(self, message, server_link=None):
        ...
        Asks Lida to join a server.  Note: Bot accounts cannot use invite links.
        """
        ...
        try:
            ...
            raise exceptions.CommandError('Invalid URL provided:\n{}\n'.format(server_link), expire_in=60)
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        async with self.aiolocks[_func_() + ':' + author.id]:
            if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
                raise exceptions.PermissionsError(
                    "You have reached your enqueued song limit (%s)" % permissions.max_songs, expire_in=60
                )
            ...
            if info.get('url', '').startswith('ytsearch'):
                ...
                if not info:
                    raise exceptions.CommandError(
                        ...
                        "You may need to restart Lida if this continues to happen.", expire_in=60
                    )
            ...
            if 'entries' in info:
                ...
                if not permissions.allow_playlists and ':search' in info['extractor'] and len(info['entries']) > 1:
                    raise exceptions.PermissionsError("You are not allowed to request playlists", expire_in=60)
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        if extractor_type == 'youtube:playlist':
            try:
                ...
                raise exceptions.CommandError('Error handling playlist %s queuing.' % playlist_url, expire_in=60)
        ...
        if not songs_added:
            ...
            raise exceptions.CommandError(basetext, expire_in=60)
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        try:
            ...
            raise exceptions.CommandError("Please quote your search query properly.", expire_in=60)
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                ...
                now_playing = discord.Embed(title="Title", description=player.current_entry.title, color=0x369167)

                # Thumbnail
                #now_playing.set_thumbnail(url=player.current_entry.meta['author'].name)

                # Fields
                now_playing.add_field(name="Progress", value=prog_str, inline=True)
                now_playing.add_field(name="Added by", value=player.current_entry.meta['author'].name, inline=True)
                now_playing.add_field(name="URL", value=player.current_entry.url, inline=True)
            else:
                '''
            ...
            self.server_specific_data[server]['last_np_msg'] = await self.send_message(channel, embed=now_playing)
    ...
    async def cmd_summon(self, channel, server, author, voice_channel):
        ...
        Call Lida to the summoner's voice channel.
        """
    ...
    async def cmd_pause(self, player):
        ...
        if player.is_playing:
            ...
            raise exceptions.CommandError('Player is not playing.', expire_in=60)
    ...
    async def cmd_resume(self, player):
        ...
        if player.is_paused:
            ...
            raise exceptions.CommandError('Player is not paused.', expire_in=60)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        Skips the current song when enough votes are cast, or by Lida owner.
        """

        if player.is_stopped:
            raise exceptions.CommandError("Can't skip! The player is not playing!", expire_in=60)
            ...
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    ...
                    print("Something odd is happening.  "
                          "You might want to restart Lida if it doesn't start working.")
            else:
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        try:
            ...
            raise exceptions.CommandError('{} is not a valid number'.format(new_volume), expire_in=60)
            ...
            if relative:
                raise exceptions.CommandError(
                    'Unreasonable volume change provided: {}{:+} -> {}%.  Provide a change between {} and {:+}.'.format(
                        old_volume, vol_change, old_volume + vol_change, 1 - old_volume, 100 - old_volume), expire_in=60)
            else:
                raise exceptions.CommandError(
                    'Unreasonable volume provided: {}%. Provide a value between 1 and 100.'.format(new_volume), expire_in=60)
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        Removes up to [range] messages Lida has posted in chat. Default: 50, Max: 1000
        """
    ...
    async def cmd_setavatar(self, message, url=None):
        ...
        if message.attachments:
            ...
            raise exceptions.CommandError("You must provide a URL or attach a file.", expire_in=60)
        ...
        try:
            ...
            raise exceptions.CommandError("Unable to change avatar: {}".format(e), expire_in=60)

        return Response("\N{OK HAND SIGN}", delete_after=20)

    async def cmd_disconnect(self, server):
        await self.disconnect_voice_client(server)
        return Response("You can summon me to your channel again by using `!summon`.", delete_after=20)
