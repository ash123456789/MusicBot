...
class Config:
    ...
    def __init__(self, config_file):
        ...
        self.lida_images_file = config.get('Files', 'LidaImagesFile', fallback=ConfigDefaults.lida_images_file)
        self.hani_images_file = config.get('Files', 'HaniImagestFile', fallback=ConfigDefaults.hani_images_file)
    ...
    async def async_validate(self, bot):
        ...
        if self.owner_id == bot.user.id:
            raise HelpfulError(
                ...
                "Lida's user ID and the id for OwnerID is identical.  "
                "This is wrong.  Lida needs its own account to function, "
                "meaning you cannot use your own account to run Lida on.  "
                "The OwnerID is the id of the owner, not Lida.  "
    ...
    def find_config(self):
        ...
        if not os.path.isfile(self.config_file):
            ...
            elif os.path.isfile('config/options.ini'):
                shutil.copy('config/options.ini', self.config_file)
                log.warning('Options file not found, copying options.ini')
        ...
        if not config.read(self.config_file, encoding='utf-8'):
            ...
            try:
                ...
                if not int(c.get('Permissions', 'OwnerID', fallback=0)): # jake pls no flame
                    print(flush=True)
                    log.critical("Please configure config/options.ini and re-run Lida.")
                    sys.exit(1)
                ...
                print(flush=True)
                log.critical("Unable to copy config/options.ini to {}".format(self.config_file), exc_info=e)
                sys.exit(2)
...
class ConfigDefaults:
    ...
    lida_images_file = 'config/images/lida.txt'
    hani_images_file = 'config/images/hani.txt'
